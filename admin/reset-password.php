...
$st = $db->prepare("SELECT * FROM admin_users WHERE reset_token=? AND status='active' LIMIT 1");
$st->execute([$token]);
$user = $st->fetch();

if (!$token || !$user) {
    $error = 'This reset link is invalid or has already been used.';
} elseif ($user['reset_expires_at'] && strtotime($user['reset_expires_at']) < time()) {
...
if (!$error && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($pass) < 8) {
        ...
        $db->prepare("UPDATE admin_users SET password_hash=?,reset_token=NULL,reset_expires_at=NULL WHERE id=?")
...
<title>Reset Password — Shimane Admin</title>
<style>
...
.box{background:#fff;border-radius:20px;box-shadow:0 8px 40px rgba(61,191,175,.15);padding:40px 36px;width:100%;max-width:420px;position:relative}
...
<div class="box">
  <div class="lang-tog">
    <a href="/admin/reset-password?token=<?= urlencode($token) ?>&amp;setlang=en" class="lt-btn <?= $lang==='en'?'on':'' ?>">EN</a>
    <a href="/admin/reset-password?token=<?= urlencode($token) ?>&amp;setlang=ja" class="lt-btn <?= $lang==='ja'?'on':'' ?>">日本語</a>
  </div>
  <div class="logo">
  ...
  <?php if ($done): ?>
    <div class="ok">✅ <strong>Password updated!</strong> You can now sign in with your new password.</div>
  ...
  <?php elseif ($error): ?>
    ...
    <p class="note"><a href="/admin/forgot-password" style="color:#3DBFAF">Request a new reset link</a></p>

  <?php else: ?>
    <h1>Set New Password</h1>
    <p class="sub">Hi <?= htmlspecialchars($user['name']) ?>, choose a new password for your account.</p>
    <?php if ($error): ?><div class="err"><?= htmlspecialchars($error) ?></div><?php endif; ?>
    <form method="POST" action="/admin/reset-password?token=<?= urlencode($token) ?>">
      <div class="fg">
